...
function beginTest(){
    ...
    $("#options").addClass("changed");
    setTimeout(function(){
        $("#question").removeClass("changed");
        $("#options").removeClass("changed");
    },1000);
    $("#question").html("Q："+questions[currentQuiz].question);
    questions[currentQuiz].answers.forEach(function(element,index){
        $("#options").append(`<label>
        <div class="oneOpt" style="width:30%; margin: 0 1%; background-color: #F8C471; cursor:pointer;">
        <input name='option' onchange="nextQ()" type='radio' style='display: none;' value='${element[1]}'>${element[0]}</div></label>`);
        });
    $("#testbtn").html("");
}

function nextQ(){
    var next = $('input[name=option]:checked').val();
    var title = "測驗結果："
    $("#options").html(" ");
    switch(next){
        case 'A':
            $("#question").html(title+"A 今年你的"+finalAnswers.A[0]);
            $("#options").html( finalAnswers.A[1]);
            break;
        case 'B':
            $("#question").html(title+"B 今年你的"+finalAnswers.B[0]);
            $("#options").html( finalAnswers.B[1]);
            break;
        case 'C':
            $("#question").html(title+"C 今年你的"+finalAnswers.C[0]);
            $("#options").html( finalAnswers.C[1]);
            break;
        case 'D':
            $("#question").html(title+"D 今年你的"+finalAnswers.D[0]);
            $("#options").html( finalAnswers.D[1]);
            break;
        default:
            $("#question").html("Q："+questions[next - 1].question);
            questions[next - 1].answers.forEach(function(element,index){
            $("#options").append(`<label>
            <div class="oneOpt" style="width:30%; margin: 0 1%; background-color: #F8C471; cursor:pointer;">
            <input name='option' onchange="nextQ()" type='radio' style='display: none;' value='${element[1]}'>${element[0]}</div></label>`);
            });
